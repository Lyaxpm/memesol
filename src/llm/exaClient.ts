...
export class ExaClient implements LlmProvider {
  ...
  async complete(req: LlmRequest): Promise<LlmResponse> {
    ...
    if (!token) {
      return { text: '{"action":"SKIP","confidence":0.4,"reasons":["Exa API key missing"],"warnings":["observation-only fallback"],"notes":"Set EXA_API_KEY before running autonomous decisions"}', model: "fallback" };
    }
    ...
    const run = async () => {
      ...
      try {
        ...
        return { text: extractAnswer(json), model: this.env.EXA_MODEL };
      } finally {
    ...
    try {
      ...
      if (err instanceof HttpError && (err.status === 401 || err.status === 403)) {
        return {
          text: '{"action":"SKIP","confidence":0.4,"reasons":["Exa authentication failed"],"warnings":["observation-only fallback"],"notes":"Check EXA_API_KEY and EXA_BASE_URL"}',
          model: "fallback"
        };
      }
      throw err;
